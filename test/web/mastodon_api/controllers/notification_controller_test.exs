...
defmodule Pleroma.Web.MastodonAPI.NotificationControllerTest do
  ...
  describe "exclude_visibilities" do
    ...
    test "filters notifications for Like activities" do
      ...
      {:ok, _, _} = CommonAPI.favorite(user, public_activity.id)
      {:ok, _, _} = CommonAPI.favorite(user, direct_activity.id)
      {:ok, _, _} = CommonAPI.favorite(user, unlisted_activity.id)
      {:ok, _, _} = CommonAPI.favorite(user, private_activity.id)
  ...
  test "filters notifications using exclude_types" do
    ...
    {:ok, favorite_activity, _} = CommonAPI.favorite(other_user, create_activity.id)
